...
client.connect(err => {
	...
	const rdb = client.db('real-star-wars-quotes')
	const rQuotesCollection = rdb.collection('quotes')
	...
	app.get('/admin', (req, res) => {
		console.log('appid: ' + process.env.APP_ID)
		rdb.collection('quotes').find().toArray()
 			.then(results => {
 				res.render('adminIndex.ejs', { quotes: results })
 			})
 			.catch(error => console.error(error))	
	})
	 ...
	 app.post('/admin/quotes', (req, res) => {
		rQuotesCollection.insertOne(req.body)
			.then(result => {
				res.redirect('/admin')
			})
			.catch(error => console.error(error))
	 ...
	 app.put('/admin/quotes', (req, res) => {
		rQuotesCollection.findOneAndUpdate(
			{ name: 'Yoda' },
			{
				$set: {
					name: req.body.name,
	 ...
	 app.delete('/admin/quotes', (req, res) => {
		rQuotesCollection.deleteOne(
			{ name: req.body.name }
		)
			.then(result => {
				if (result.deletedCount == 0) {
